...
import delBookReq from "@/app/actions/BookReq/delBookReq";
const KitapOneriClient = (props) => {
  const pdata = props.bookoneri || [];
  ...
  const oneriDel = async (item) => {
    Swal.fire({
      title: item.name + " Adlı Öneri Silinecek!! ",
      showDenyButton: true,
      confirmButtonText: "Sil",
    }).then(async (result) => {
      if (result.isConfirmed) {
        const res = await delBookReq(item);
        if (res === true) {
          await Swal.fire({
            icon: "success",
                ...
                <Body>
                  {tableList?.map((item) => {
                    return (
                      <Row key={item?.id}>
                        <Cell>{item?.name}</Cell>
                        <Cell>{item?.kitap}</Cell>
                        <Cell>{item?.yazar}</Cell>
                        <Cell>{item?.yayinevi}</Cell>
                        <Cell>
                          <button onClick={() => oneriDel(item)}>
...
export default KitapOneriClient;
