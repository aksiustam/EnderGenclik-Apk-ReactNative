...
import delYazarlar from "../actions/Yazarlar/delYazarlar";
const YazarlarClient = (props) => {
  const pdata = props.yazarlar || [];
  ...
  const yazarDel = async (item) => {
    Swal.fire({
      title: item.name + " Adlı Yazar Silinecek!! ",
      showDenyButton: true,
      confirmButtonText: "Sil",
    }).then(async (result) => {
      if (result.isConfirmed) {
        const res = await delYazarlar(item);
        if (res === true) {
          await Swal.fire({
            icon: "success",
                  ...
                  {tableList?.map((item) => {
                    return (
                      <Row key={item?.id}>
                        <Cell>#{item.id}</Cell>
                        <Cell>{item?.name}</Cell>
                        <Cell>{item?.tarih}</Cell>
                        <Cell> {item?.text}</Cell>
                        <Cell>
                          <button
                            onClick={() => router.push(`/yazarlar/${item.id}`)}
                          >
                            <FaRegEdit size={26} color="green" />
                          </button>
                          <button onClick={() => yazarDel(item)}>
